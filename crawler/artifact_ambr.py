...
def append_image(data_dict, artifact_id):
    ...
    util.download_amber_image(path, 'reliquary/' + icon, img_ext, base_dir)
...
def generate_json(artifact_id):
    ...
    with open(base_dir + '/lib/artifactlist.json', 'r', encoding='utf-8') as f:
    ...
    with open(base_dir + '/lib/artifact/' + result['id'] + '.json', 'w', encoding='utf-8') as f:
