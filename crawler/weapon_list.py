...
with open(base_dir + '/lib/weaponlist.json', 'r', encoding='utf-8', newline='\n') as f:
    exist_list = json.load(f)['data']

weapons = []
exist_ids = []
for w in exist_list:
    if 'spoiler' in w and not w['spoiler']:
        weapons.append(w)
        exist_ids.append(w['id'])
...
for k in data_dict['en']:
    ...
    util.download_amber_image('crawler/ambr/' + path, data_en['icon'], img_ext, base_dir)
    result['imageurl'] = '%s.%s' % (path, img_ext)
    result['rarity'] = data_en['rank']
    result['type'] = data_en['type'].split('_')[1].lower()
    find = list(filter(lambda wp: wp['id'] == k, exist_list))
...
with open(base_dir + '/crawler/ambr/lib/weaponlist.json', 'w', encoding='utf-8', newline='\n') as f:
