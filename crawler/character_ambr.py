...
f = open(base_dir + '/lib/character_base_data.json', 'r', encoding='utf-8', newline='\n')
...
def append_image(data_dict, character_id, size):
    ...
    util.download_amber_image('crawler/ambr/' + path, large_icon if size == 'large' else icon, img_ext, base_dir)
...
def append_talent_image(character_id, cur_talent, talent_dict, key):
    ...
    util.download_amber_image('crawler/ambr/' + path, talent_dict['en'][key]['icon'], img_ext, base_dir)
...
def append_constellation_image(character_id, cur_constellation, constellation_dict, key):
    ...
    util.download_amber_image('crawler/ambr/' + path, constellation_dict['en'][key]['icon'], img_ext, base_dir)
...
def generate_json(character_id):
    ...
    print('generate lib json from ambr for: %s %s' % (character_id, character['ENname']))
    data_dict = {}
    for lang in languages:
    ...
    with open(base_dir + '/crawler/ambr/lib/character/' + result['id'] + '.json', 'w', encoding='utf-8', newline='\n') as f:
